...
- [ ] **A5. `llm-task` JSON-Only Classification** — Blocked: the bundled plugin loads and appears in `tools.catalog`, but direct `tools.invoke` returns `Tool not available: llm-task`; agent-mediated invocation hangs until the outer timeout.
- [ ] **A6. `llm-task` Tool Isolation** — Blocked by A5. Do not rely on `llm-task` for prompt-injection isolation until a direct invocation path completes and can be verified.
...
**Go/No-Go**: A1-A4 and A7 validate plugin loading, tool invocation, TypeBox parameter validation, and sub-agent orchestration. A5/A6 are architecture-critical blockers for using bundled `llm-task` as the JSON-only prompt-injection isolation layer. A8 currently validates scheduling persistence/firing but not plugin-tool execution from cron; production may need an agent message that uses globally allowed tools, a core-tool cron path, or an external scheduler until this is resolved. If A1-A3 fail, we need a fundamentally different orchestration approach.
