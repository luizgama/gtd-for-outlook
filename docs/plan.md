...
- [x] **A4. TypeBox Tool Parameters** — Register a tool with `@sinclair/typebox` schema parameters (String, Number, Optional, Union, Literal). Invoke via agent. Verify parameter validation works — valid params succeed, invalid params are rejected.
- [x] **A5. `llm-task` JSON-Only Classification** — Configure `plugins.entries.llm-task.enabled=true` and `tools.allow=["echo_tool","typed_echo_tool","llm-task"]`, then call it directly through `openclaw gateway call tools.invoke`. Normal and adversarial email inputs returned schema-valid JSON. Prompt must explicitly require every schema key.
- [x] **A6. `llm-task` Tool Isolation** — With `echo_tool`, `typed_echo_tool`, and `llm-task` all allowed globally, direct `llm-task` invocation against adversarial input returned schema-valid JSON with `attemptedToolCall=true` and `toolCalled=false`. The bundled implementation also runs the embedded agent with `disableTools: true`.
- [x] **A7. Sub-Agent Orchestration** — Parent `main` dynamically spawned sub-agent sessions, yielded, resumed, and returned Agent-A → Agent-B handoff output without requiring named agents beyond `main`.
- [ ] **A8. Cron Scheduler** — Partially validated: the scheduler persisted the job and fired repeatedly, but plugin-tool cron execution failed with `runtime toolsAllow: echo_tool` because plugin tools were not resolved under the cron runtime allow-list.
- [ ] **A9. Session Isolation** — Blocked by provider quota during concurrent validation (`openai-codex/gpt-5.5` usage limit/cooldown). Retry when model quota is available.

**Go/No-Go**: A1-A7 validate plugin loading, tool invocation, TypeBox parameter validation, `llm-task` JSON/schema output, `llm-task` tool isolation, and sub-agent orchestration. A8 currently validates scheduling persistence/firing but not plugin-tool execution from cron; production may need an agent message that uses globally allowed tools, a core-tool cron path, or an external scheduler until this is resolved. If A1-A3 fail, we need a fundamentally different orchestration approach.
